export default function getWebGLContext(canvas: HTMLCanvasElement) {
  ...
  if (webgl2) {
    ...
    if (!halfFloat) throw new Error("half_float not supported");
...
function getSupportedFormat(gl: WebGL2RenderingContext, internalFormat: number, format: number, type: number): SupportedFormat {
